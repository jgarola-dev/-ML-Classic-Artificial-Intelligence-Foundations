...
if page == "🏠 Inicio":
    ...
    if st.session_state.df is None:
        ...
        if 'abandono' not in df.columns:
            ...
            # ✅ CORRECCIÓN: Cálculo directo y sin lógica condicional errónea
            count_1 = int((df['abandono'] == 1).sum())  # Abandono = 1
            count_0 = int((df['abandono'] == 0).sum())  # No Abandona = 0
            total = len(df)
            dropout_rate = (count_1 / total) * 100

            # KPIs (mostrar UNA sola vez)
            ...
            if numeric_key_cols:
                ...
                st.download_button("📥 Descargar Estadísticas (CSV)", stats_df.to_csv(), "estadisticas_uci.csv", "text/csv")
            ...
            st.caption(f"📊 Clase 0 (No Abandona): {count_0} registros ({(count_0/total)*100:.1f}%) | Clase 1 (Abandona): {count_1} registros ({dropout_rate:.1f}%)")
            
            st.subheader("🔗 Matriz de Correlación (Pearson)")
            if len(numeric_key_cols) >= 2:
                corr = df[numeric_key_cols + ['abandono']].corr()
                st.plotly_chart(px.imshow(corr, text_auto='.2f', color_continuous_scale='RdBu_r', title="Correlaciones entre Variables Clave"), use_container_width=True)
            ...
            try:
                ...
                for i, (name, res) in enumerate(st.session_state.results.items()):
                    with cols[i]:
                        ...
                        if prob > 0.60: risk, delta_color = "🔴 ALTO RIESGO", "inverse"
                        elif prob > 0.35: risk, delta_color = "🟡 RIESGO MODERADO", "normal"
                        else: risk, delta_color = "🟢 BAJO RIESGO", "normal"
