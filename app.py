...
def map_uci_to_simple(df):
    ...
    column_mapping = {
        ...
        'unemployment_rate': 'horas_estudio', 'inflation_rate': 'socioeconomico',
...
def get_key_columns(df):
    return [c for c in ['edad', 'gpa', 'asistencia', 'horas_estudio', 'abandono'] if c in df.columns]
...
if uploaded_file is not None:
    try:
        ...
        if 'abandono' not in df_up.columns:
            found_target = next((c for c in ['target', 'status', 'estado', 'y'] if c in df_up.columns), None)
            if found_target:
                df_up['abandono'] = df_up[found_target].apply(lambda x: 1 if 'dropout' in str(x).lower() or str(x) in ['1', 'si'] else 0)
            else:
...
if page == "🏠 Inicio":
    ...
    if st.session_state.df is None:
        ...
        # ✅ CÁLCULO ÚNICO Y CORRECTO DE TASA DE ABANDONO
        count_1 = int((df['abandono'] == 1).sum())
        count_0 = int((df['abandono'] == 0).sum())
        total = len(df)
        dropout_rate = (count_1 / total) * 100
        ...
        if numeric_key_cols:
            ...
            st.download_button("📥 Descargar Estadísticas (CSV)", stats_df.to_csv(), "estadisticas.csv", "text/csv")
        
        st.subheader("🥧 Distribución de Clases")
        labels_map = {0: "No Abandona (0)", 1: "Abandona (1)"}
        labels = [labels_map.get(0, "Clase 0"), labels_map.get(1, "Clase 1")]
        fig_pie = px.pie(values=[count_0, count_1], names=labels, title="Proporción de Clases", hole=0.4, 
                         color_discrete_map={'No Abandona (0)': '#00c853', 'Abandona (1)': '#d32f2f'})
        fig_pie.update_traces(textposition='inside', textinfo='percent+label')
        st.plotly_chart(fig_pie, use_container_width=True)
        st.caption(f"📊 Clase 0: {count_0} | Clase 1: {count_1}")
            ...
            if selected_var:
                ...
                with c1:
                    fig_box = px.box(df, x='abandono', y=selected_var, color='abandono',
                                    title=f"Distribución de {selected_var.upper()}",
                                    color_discrete_map={0: '#00c853', 1: '#d32f2f'})
                    st.plotly_chart(fig_box, use_container_width=True)
                with c2:
                    fig_hist = px.histogram(df, x=selected_var, nbins=30, title=f"Histograma de {selected_var.upper()}")
    ...
    if st.session_state.df is None or 'abandono' not in st.session_state.df.columns:
        ...
        st.info(f"📋 División: ✅ Entrenamiento: {n_train} | 🔍 Prueba: {n_test}")
        ...
        if st.session_state.results:
            ...
            st.plotly_chart(px.bar(plot_df, x='Modelo', y='Valor', color='Métrica', barmode='group', title="Comparación"), use_container_width=True)
            ...
            if imp_df is not None and not imp_df.empty:
                ...
                fig = px.bar(imp_df, x='importance', y='feature', orientation='h', title=f"Top Features ({sel_model.replace('_',' ').title()})")
    ...
    if st.session_state.prep is None:
        ...
        with c1:
            ...
            horas = st.slider("⏱️ Horas Estudio", 0, 30, 10)
        with c2:
            ...
            try:
                ...
                for i, (name, res) in enumerate(st.session_state.results.items()):
                    with cols[i]:
                        ...
                        elif prob > 0.35: risk, dc = "🟡 MEDIO", "normal"
...
st.markdown("---")
st.markdown("""<div style='text-align:center;color:#6c757d;font-size:0.85rem;'>
<p>🎓 ML Clásico - Predicción de Abandono Escolar | Fundació URV</p>
<p>Dataset: UCI Student Dropout | Streamlit + Scikit-learn + Plotly</p></div>""", unsafe_allow_html=True)
