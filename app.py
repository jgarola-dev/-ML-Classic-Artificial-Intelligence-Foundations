...
def map_uci_to_simple(df):
    """Mapea columnas del dataset UCI a nombres simplificados"""
    df_mapped = df.copy()
    column_mapping = {
...
if page == "🏠 Inicio":
    ...
    if st.session_state.df is None:
        ...
        if 'abandono' not in df.columns:
            ...
            # ✅ DISTRIBUCIÓN DE ABANDONOS (%) - CORREGIDO CON .sort_index()
            st.subheader("🥧 Distribución de Abandonos (%)")
            abandono_counts = df['abandono'].value_counts().sort_index()  # 🔑 ORDENA POR ETIQUETA (0,1), NO POR FRECUENCIA
...
st.markdown("---")
st.markdown("<div style='text-align:center;color:#6c757d;font-size:0.85rem'><p>🎓 ML Clásico | Fundació URV | Dataset: UCI Student Dropout</p></div>", unsafe_allow_html=True)
