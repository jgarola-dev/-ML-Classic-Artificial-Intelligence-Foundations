...
def map_uci_to_simple(df):
    ...
    if 'abandono' in df_mapped.columns:
        if df_mapped['abandono'].dtype in ['int64', 'float64']:
            # Mapeo explícito para valores numéricos del UCI: 0=Dropout→1, 1/2=Enrolled/Graduate→0
        ...
        try:
            ...
            if target_col:
                if df_proc[target_col].dtype in ['int64', 'float64']:
                    # Mapeo explícito para valores numéricos del UCI
    ...
    if st.session_state.df is None:
        ...
        if 'abandono' not in df.columns:
            ...
            if df['abandono'].dtype == 'object':
                ...
                # Contar explícitamente la clase 1 (Abandona)
            ...
            # Mapeo explícito de etiquetas basado en el valor REAL de la clase
...
""", unsafe_allow_html=True):0.85rem'><p>🎓 ML Clásico | Fundació URV | Dataset: UCI Student Dropout</p></div>", unsafe_allow_html=True)
